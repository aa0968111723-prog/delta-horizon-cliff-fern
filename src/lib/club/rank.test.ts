...
import { hitScore, matchHit, rankHits, searchKeys, clubTagsFromText } from "./rank.ts";
...
test("unspaced Drive questions still keep tea and night keys", () => {
  ...
  assert.ok(clubTagsFromText("夜間茶會照片").includes("茶會"));
  assert.ok(clubTagsFromText("夜間茶會照片").includes("晚上"));
});
