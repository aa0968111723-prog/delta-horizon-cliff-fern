...
function score(post: IgMemoryPost) {
  return (post.saves ?? 0) * 4 + (post.shares ?? 0) * 3 + (post.comments ?? 0) * 2 + (post.likes ?? 0) * 0.2 + (post.reach ?? 0) * 0.01;
}
...
export function learnFromIg(posts: IgMemoryPost[]): IgLearning {
  ...
  if (top) {
    lessons.push({
      id: "hook",
      title: "哪種 Hook 比較有效？",
      detail: `收藏較高的是「${firstLine(top.caption)}」。問句與生活語氣比社團全名更容易停。`,
    });
  }
  lessons.push({
    id: "visual",
  ...
  lessons.push({
    ...
    detail: "封面一句話 → 情境 → 痛點 → 內容 → CTA。每頁只做一件事。",
  });
  lessons.push({
    id: "story",
    title: "Story 哪種互動比較多？",
  ...
  const promptBlock = [
    ...
    top ? `收藏 ${top.saves ?? 0}、留言 ${top.comments ?? 0}` : "",
    avoid,
  ...
  return { ranked, lessons, bestHookShape, avoid, promptBlock };
}
