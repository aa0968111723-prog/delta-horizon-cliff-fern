...
set-format{formatId:feed-square|feed-portrait|feed-landscape|story|reels-cover|threads|line,label}
...
畫布：${JSON.stringify(scene)}`;
  ...
  if (!res.ok) {
    return { ok: false, error: `畫布代理暫時無法使用（${res.status}）。可改用本機規則。`, adapter: "live" };
  }
