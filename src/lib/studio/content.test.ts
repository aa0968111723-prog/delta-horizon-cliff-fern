...
import { inferContentKind, kindFromFormat, statusFromLegacy } from "./content.ts";
...
test("inferContentKind uses slides length", () => {
  const kind = inferContentKind({
    ...
    slides: { "feed-portrait": [{ formatId: "feed-portrait", background: { type: "solid", color: "#000" }, layers: [] }, { formatId: "feed-portrait", background: { type: "solid", color: "#000" }, layers: [] }] },
  });
