...
export const startInstagramConnect = createServerFn({ method: "POST" })
  .validator((input: unknown) => ConnectSchema.parse(unwrapServerInput(input) ?? {}))
  .handler(async ({ data }): Promise<ConnectorResult<{ url: string }>> => {
    ...
    return startInstagramOAuthUrl({ includeInsights: Boolean(data.insights) });
  });
