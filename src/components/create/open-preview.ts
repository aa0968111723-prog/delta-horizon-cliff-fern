...
import { isWaveScheduleItem, schedulePreviewAssetId } from "@/lib/zen/schedule";
...
export function openScheduledPreview(item: ScheduleItem) {
  ...
  if (isWaveScheduleItem(item)) {
    const assetId = schedulePreviewAssetId(item, useCreative.getState().campaigns);
    if (item.projectId) useStudio.getState().setLastProjectId(item.projectId);
