...
import { _ as stampSlideMeta, c as completeCarouselPages, d as formatById, g as roleTemplate, h as remapLayerToFormat, l as copyForCarouselPage, m as pagePlanFromArtboard, o as PAGE_ROLE_LABEL$1, p as isCarouselRole, r as CAROUSEL_SEQUENCE, u as copyFromArtboard } from "./schema-iJpgBEjq.mjs";
import { u as require_react } from "../_libs/@floating-ui/react-dom+[...].mjs";
import { A as Slot, P as require_jsx_runtime } from "../_libs/@radix-ui/react-alert-dialog+[...].mjs";
import { n as clsx, t as cva } from "../_libs/class-variance-authority+clsx.mjs";
import { t as twMerge } from "../_libs/tailwind-merge.mjs";
import { a as inferContentKind, o as migrateStatus, t as CONTENT_KIND_META } from "./status-BrzQjlVh.mjs";
import { _ as eventKindLabel, a as CLUB_HANDLE, c as CLUB_NAME, d as CLUB_SLOGANS, f as CLUB_VOICE, h as VISUAL_ANCHORS, i as CLUB_DO_SAY, l as CLUB_PALETTE, m as MASCOT, n as CLUB_CTAS, o as CLUB_HASHTAGS, r as CLUB_DONT_SAY, s as CLUB_INTRO_SHORT } from "./club-CaoIGk4S.mjs";
...
//#endregion
//#region node_modules/.nitro/vite/services/ssr/assets/studio-store-BqpaoTm7.js
...
function campaignDateMs(campaign) {
...
/** 排序：即將到來的優先，接著是最近結束的。 */
function sortByUpcoming(list, now = Date.now()) {
	return [...list].sort((a, b) => {
		...
		if (da == null) return 1;
		if (db == null) return -1;
		return (da >= 0 ? da : 1e3 - da) - (db >= 0 ? db : 1e3 - db);
	});
}
function nextCampaign(list, now = Date.now()) {
	return sortByUpcoming(list, now).filter((c) => {
...
* 避免 IG 看起來一直在招生。
*/
function defaultWavePlan(campaign) {
	...
	return rows.filter((row) => row.offsetDays >= 0 || Math.abs(row.offsetDays) <= Math.max(3, window)).map((row) => ({
		...row,
		id: uid("wave"),
		contentId: null
	}));
}
function waveDateMs(campaign, wave) {
	const base = campaignDateMs(campaign);
	if (base == null) return null;
	return base + wave.offsetDays * DAY;
}
function waveDateLabel(campaign, wave) {
...
var SEED_BRAND_ID = "brand_tku_zen";
var SEED_PROJECT_ID = "content_fuyou_cover";
var SEED_DRAFT_ID = "content_midterm_breath";
var SEED_LOGO_ID = "asset_zen_mark";
var SEED_MASCOT_ID = "asset_gugu";
var SEED_DUSK_ID = "asset_tamsui_dusk";
var SEED_WINDOW_ID = "asset_window_light";
var SEED_NIGHT_ID = "asset_night_lamp";
var SEED_CAMPAIGN_ID = "camp_fuyou_zen_light";
var SEED_TIME = Date.parse("2026-09-08T00:00:00+08:00");
var SEED_ASSETS = [
	{
		id: SEED_LOGO_ID,
		name: "三色光標誌",
...
var SEED_BRAND = {
	...
	colors: [
		{
			id: "c1",
			hex: CLUB_PALETTE.clear,
			role: "primary",
			label: "澄光"
		},
		{
			id: "c2",
			hex: CLUB_PALETTE.night,
			role: "secondary",
			label: "夜光"
		},
		{
			id: "c3",
			hex: CLUB_PALETTE.paper,
			role: "background",
			label: "紙白"
		},
		{
			id: "c4",
			hex: CLUB_PALETTE.warm,
			role: "accent",
			label: "曦光"
		},
		{
			id: "c5",
			hex: CLUB_PALETTE.ink,
			role: "ink",
			label: "深墨綠"
		}
	],
...
function createSeedProject() {
	const now = SEED_TIME;
	const page1 = buildLayout("feed-portrait", copy, SEED_BRAND, "editorial", { imageAssetId: SEED_DUSK_ID });
	...
	const page2 = buildLayout("feed-portrait", {
		...
		cta: "來坐一下"
	}, SEED_BRAND, "quote", { imageAssetId: SEED_NIGHT_ID });
	...
	const page3 = buildLayout("feed-portrait", {
		...
		cta: "社課見"
	}, SEED_BRAND, "editorial", { imageAssetId: SEED_WINDOW_ID });
	...
	const plan = migratePlan({
		campaignName: "浮游禪光",
		concept: "開學一個多月，學生開始累但還說不出來。用「你不是懶，只是很久沒休息」切進去，再把活動說成「一小時的空白」，而不是社團招生。",
...
function createSeedDraft() {
	...
	const artboard = buildLayout("story", draftCopy, SEED_BRAND, "quote", { imageAssetId: SEED_NIGHT_ID });
...
function createSeedCampaign() {
	const base = migrateCampaign({
		...
		intro: "「浮游禪光」是一場給完全沒接觸過靜坐的人的體驗。前十五分鐘由社員帶著坐，之後想繼續坐、想講話、想安靜都可以。不用盤腿、不用穿特別的衣服，直接來就好。",
	...
	const waves = defaultWavePlan(base).map((wave) => wave.offsetDays === -7 ? {
		...wave,
		contentId: SEED_PROJECT_ID
	} : wave);
	return {
		...base,
		waves
...
var SEED_CAMPAIGNS = [createSeedCampaign()];
createSeedProject();
...
var STORAGE_KEY = "tku-zen-studio-v1";
...
function migrateBrandRecord(raw) {
	...
	if (next.id !== "brand_tku_zen") return next;
	return {
		...next,
...
var useStudio = create()(persist((set, get) => ({
	...
	createCampaign: (patch) => {
		const campaign = migrateCampaign({
			...patch,
			id: patch?.id ?? uid("camp")
		});
	...
	deleteCampaign: (id) => set((s) => ({
		...
		projects: s.projects.map((p) => p.campaignId === id ? {
			...p,
			campaignId: null
		} : p)
	})),
	setCampaignWaves: (id, waves) => get().updateCampaign(id, { waves }),
	updateWave: (campaignId, waveId, patch) => set((s) => ({ campaigns: s.campaigns.map((c) => c.id === campaignId ? {
		...c,
		waves: c.waves.map((w) => w.id === waveId ? {
			...w,
			...patch
		} : w),
		updatedAt: Date.now()
	...
	fillDefaultWaves: (id) => {
		...
		get().updateCampaign(id, {
			waves: defaultWavePlan(campaign),
			planSource: "mock"
		});
	},
	setContentKind: (projectId, kind) => {
		...
		if (meta && meta.formatId !== get().projects.find((p) => p.id === projectId)?.activeFormatId) get().setActiveFormat(projectId, meta.formatId);
	},
	setSchedule: (projectId, at) => get().updateProject(projectId, (p) => ({
		...p,
		scheduledAt: at,
		status: at ? p.status === "published" ? p.status : "scheduled" : p.status === "scheduled" ? "done" : p.status
	})),
	markPublished: (projectId, at) => get().updateProject(projectId, (p) => ({
		...p,
		status: "published",
		publishedAt: at ?? Date.now()
	})),
	addCopyDraft: (projectId, draft) => get().updateProject(projectId, (p) => ({
		...p,
	...
	useCopyDraft: (projectId, draftId) => {
		...
		get().updateProject(projectId, (p) => ({
			...
			copy: {
				...p.copy,
				caption,
				hashtags: draft.hashtags.length ? draft.hashtags : p.copy.hashtags
			},
	...
	addSources: (projectId, sources) => get().updateProject(projectId, (p) => {
...
//#endregion
export { Button as $, formatCampaignDate as A, pagesOf as B, cssFilter as C, defaultWavePlan as D, daysUntil as E, matchesAssetQuery as F, snapMove as G, resizeBox as H, migrateBrief as I, textOverflows$1 as J, sortByUpcoming as K, nextCampaign as L, inspectProject as M, kindFromCategory as N, emptyBrief as O, logoUsageLabel as P, waveDateMs as Q, normalizeCrop as R, createTextLayer as S, cssTextShadow as T, resolveProjectId as U, previewTemplate as V, rotateByPointer as W, useStudio as X, usageLabel as Y, waveDateLabel as Z, createGeneratedAsset as _, DELIVERABLE_OPTIONS as a, createLogoLayer as b, TEMPLATE_META as c, assetUsageStatus as d, buttonVariants as et, campaignDateMs as f, countdownLabel as g, collectUsedAssetIds as h, ASSET_SOURCES as i, formatsFromBrief as j, emptyCampaign as k, TEMPLATE_STARTERS as l, categoryLabel as m, ASSET_DRAG_MIME as n, LOGO_USAGE as o, campaignTitle as p, sourceLabel as q, ASSET_NEED_LABEL as r, PAGE_ROLE_LABEL as s, ASSET_CATEGORIES as t, cn as tt, activeArtboard as u, createImageLayer as v, cssShadow as w, createShapeLayer as x, createLineLayer as y, normalizeFilter as z };
