//#region node_modules/.nitro/vite/services/ssr/assets/audience-Bhnj2Vl6.js
...
function resolveAudience(ids) {
	const list = ids.map(audienceById).filter((item) => Boolean(item));
	return list.length ? list : DEFAULT_AUDIENCE_IDS.map((id) => audienceById(id)).filter(Boolean);
}
/** 給 AI prompt 用的一段人物描述。 */
function describeAudience(ids) {
	return resolveAudience(ids).map((seg) => `- ${seg.label}：${seg.situation} 痛點：${seg.pain} 會停下來的話：${seg.trigger}`).join("\n");
}
//#endregion
export { DEFAULT_AUDIENCE_IDS as n, describeAudience as r, AUDIENCE_SEGMENTS as t };
